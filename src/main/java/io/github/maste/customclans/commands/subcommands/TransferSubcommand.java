...
public final class TransferSubcommand extends AbstractClanSubcommand {
    ...
    public TransferSubcommand(
        ...
        super(plugin, messages, "transfer", "clans.manage", true);
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        handleAction(sender, clanService.transferLeadership(asPlayer(sender), args[0]), result -> {
            ClanMember clanMember = result.value();
            Player target = plugin.getServer().getPlayer(clanMember.playerUuid());
            if (target != null) {
                messages.send(
                        target,
                        "transfer.success-target",
